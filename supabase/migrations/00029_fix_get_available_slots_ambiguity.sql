-- Fix: "column reference is_available is ambiguous" in get_available_slots.
-- The RETURNS TABLE column `is_available` clashes with the
-- `availability_overrides.is_available` column inside the IF EXISTS check.
-- Solution: add table alias `ao` and fully qualify the column reference.
