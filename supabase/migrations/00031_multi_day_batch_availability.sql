-- Batch function: for each doctor in the input array, find the next N available
-- days (up to p_lookahead_days ahead) and return up to p_max_slots_per_day slots
-- for each day. Returns flat rows (doctor_id, available_date, slot_start, slot_end).
-- Based on get_next_available_slots_batch (00027) but returns multiple days.

CREATE OR REPLACE FUNCTION public.get_multi_day_available_slots_batch(
  p_doctor_ids UUID[],
  p_lookahead_days        INT  DEFAULT 14,
  p_max_days_per_doctor   INT  DEFAULT 3,
  p_max_slots_per_day     INT  DEFAULT 4,
...
BEGIN
  ...
  -- 2. Generate candidate dates (today .. today + p_lookahead_days) per doctor
  candidate_dates AS (
    SELECT dt.did,
           dt.tz,
    ...
    FROM   doctor_tz dt
    CROSS JOIN generate_series(0, p_lookahead_days - 1) AS d_offset
  ),
  ...
  -- 6. Rank: per doctor, rank days by date, rank slots within each day
  ranked AS (
    SELECT a.did,
           a.cdate,
           a.s_start,
           a.s_end,
           DENSE_RANK() OVER (PARTITION BY a.did ORDER BY a.cdate) AS day_rank,
           ROW_NUMBER()  OVER (PARTITION BY a.did, a.cdate ORDER BY a.s_start) AS slot_rn
    FROM   available a
  )
  ...
  SELECT r.did      AS doctor_id,
         ...
         r.s_end    AS slot_end
  FROM   ranked r
  WHERE  r.day_rank  <= p_max_days_per_doctor
    AND  r.slot_rn   <= p_max_slots_per_day
  ORDER BY r.did, r.cdate, r.s_start;
END;
$$;
